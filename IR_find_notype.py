...
class TMUCrawler:
    ...
    def check_content(self, html, exclude_texts):
        """檢查頁面內容是否包含任何排除文字"""
        if html is None:
            return False
        # 如果任何一個排除文字出現在頁面中，返回False
        return not any(text in html for text in exclude_texts)
    ...
    def save_results(self, filename='notype_results.csv'):
        """保存結果到CSV文件"""
...
def get_exclude_texts():
    """獲取使用者輸入的排除文字列表"""
    exclude_texts = []
    print("請輸入要排除的文字（每行一個，輸入空行結束）：")
    while True:
        text = input().strip()
        if not text:  # 如果輸入空行，結束輸入
            break
        exclude_texts.append(text)
    return exclude_texts

def main():
    crawler = TMUCrawler()
    
    # 獲取使用者輸入的排除文字
    exclude_texts = get_exclude_texts()
    print(f"將排除包含以下文字的頁面：{exclude_texts}")
    ...
    # 開始爬蟲
    crawler.crawl(start_id, end_id, exclude_texts)
